    ...
    <div>
      <section className="bg-gray-900 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl font-bold tracking-tight mt-4">Plan your next trip, all in one place</h1>
          ...
          <div className="max-w-3xl mx-auto mt-8 bg-white rounded-2xl p-2 flex flex-col sm:flex-row gap-2 shadow-xl shadow-black/10">
            <input
              ...
              onKeyDown={(e) => { if (e.key === "Enter") startAISearch(); }}
              ...
              maxLength={500}
            />
            <Button className="sm:w-36" onClick={startAISearch} leftIcon={<Search className="w-4 h-4" />}>
              Ask SAFARNI
            </Button>
          </div>
          <Link href="/ai-search" className="inline-block mt-3 text-xs text-gray-400 hover:text-white">Or open the full AI travel search</Link>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-10">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 grid grid-cols-3 sm:grid-cols-6 gap-2">
          {categories.map((cat) => (
            <Link key={cat.href} href={cat.href} className="flex flex-col items-center gap-2 py-4 rounded-lg hover:bg-gray-50 transition-colors">
              <div className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center"><cat.icon className="w-5 h-5 text-gray-700" /></div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <SectionHeader title="Featured packages" description="Curated bundles with a built-in discount" viewAllHref="/packages" />
        <ListingGrid
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <SectionHeader title="Popular tours" description="Hand-picked experiences travelers love" viewAllHref="/tours" />
        <ListingGrid
