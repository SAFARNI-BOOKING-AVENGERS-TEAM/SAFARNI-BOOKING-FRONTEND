...
export default function FlightBookingModal({ isOpen, onClose, flight }: FlightBookingModalProps) {
  ...
  const handleSubmit = () => {
    ...
    createBooking.mutate(
      ...
      {
        onSuccess: () => {
          toast.success("Flight booked!", `${flight.airline} ${flight.flightNumber}`);
          onClose();
        },
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Book ${flight.flightNumber}`}>
      <p className="text-sm text-gray-500 mb-4">
        {flight.departureAirport} → {flight.arrivalAirport} · {formatDate(flight.departureTime)} at{" "}
      ...
      <div className="mt-4 pt-4 border-t border-gray-100 space-y-2">
        <div className="flex justify-between text-sm text-gray-500">
          <span>
            {formatPrice(flight.price)} × {passengers} passenger{passengers === 1 ? "" : "s"}
          </span>
