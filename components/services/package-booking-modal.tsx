...
export default function PackageBookingModal({
  ...
  const handleSubmit = () => {
    ...
    bookPackage.mutate(payload, {
      onSuccess: () => {
        toast.success("Package booked!", packageTitle);
        onClose();
      },
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Book ${packageTitle}`} size="lg">
          ...
          <div key={idx} className="border border-gray-200 rounded-lg p-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <FormInput
                label="Start"
                type="date"
                value={dates[idx]?.startDate ?? ""}
                onChange={(e) => updateDate(idx, "startDate", e.target.value)}
              />
              <FormInput
                label="End"
                type="date"
                min={dates[idx]?.startDate}
                value={dates[idx]?.endDate ?? ""}
      ...
      <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between text-base font-semibold text-gray-900">
        ...
        <span>{formatPrice(estimatedFinalPrice)}</span>
      </div>

      <Button className="w-full mt-4" onClick={handleSubmit} isLoading={bookPackage.isPending}>
