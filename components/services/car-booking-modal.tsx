...
export default function CarBookingModal({ isOpen, onClose, car }: CarBookingModalProps) {
  ...
  const handleSubmit = () => {
    ...
    createBooking.mutate(
      ...
      {
        onSuccess: (response) => {
          toast.success("Booking created", "Complete payment to confirm your car rental.");
          onClose();
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Book ${car.brand} ${car.model}`}>
      <DateRangeFields startDate={startDate} endDate={endDate} onStartDateChange={setStartDate} onEndDateChange={setEndDate} startLabel="Pick-up" endLabel="Drop-off" />
      <div className="mt-4 pt-4 border-t border-gray-100 space-y-2">
        <div className="flex justify-between text-sm text-gray-500">
          <span>{formatPrice(car.pricePerDay)} × {days} day{days === 1 ? "" : "s"}</span>
